...
for (tissue in tissues) {
  ...
  if (is.null(subtypes)==FALSE){
    for (subtype in subtypes) {
      ...
      ntum<-traw2[,tcgacodes=="11A"]
      colnames(ntum) <- substr(colnames(ntum),1,16)
      ...
      rawcounts <- cbind(tumor,ntum,gnorm)
      ...
      batch<-factor(c(rep("1",ncol(tumor)),rep("1",ncol(ntum)),rep("2",ncol(gnorm))))
      names(batch)<-colnames(rawcounts)
      ...
      if(!file.exists(paste0("data/GTEx_",name,"_",subtype,"-regulon.rda"))){
        ...
        regulon<-corto(expmat[,colnames(gnorm)],centroids=surfacer,nbootstraps = 1000,p=1e-08,nthreads=2,
                       verbose = TRUE)
